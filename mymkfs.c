...
void mymkfs(int total_size) {
    ...
    sb.num_inodes =
        (avaible_size / 10) / (sizeof(inode)); // we want the all size of inodes
    // to be 10 % of the superblock
...
int mymount(const char *source, const char *target, const char *filesytemtype,
...
int myopen(const char *pathname, int flags) {
    int ans = find_fileodir(pathname, FILE_T,
                            flags); // get the file descriptor for the wanted file
...
off_t mylseek(int myfd, off_t offset, int whence) {
    ...
    if (openfiles[myfd].filenum == -1) {
        printf("from mylseek : you need to open the file before to moove the "
               "seek_pointer\n");
        return -1;
    }
    ...
    switch (whence) {
        ...
        case 1: // case of SEEK_CUR
            // printf("from mylseek we : moove the pointer to the curr pos of %d to %d
            // char\n",openfiles[myfd].seek_pos , openfiles[myfd].seek_pos + offset);
            openfiles[myfd].seek_pos += offset;
            break;
        case 2:
            // printf("from mylseek we : moove the pointer to the end pos at %d to %d
            // char\n",dbs[inodes->first_block].last_pos ,
            // dbs[inodes->first_block].last_pos - offset );
...
int find_fileodir(const char *pathname, int type,
                  int flag) { // type = 1 for folder , type = 0 for file
    char separ = '/';
    ...
    if (nbr_dir > 2) {
        while (curr_dir != NULL && j < nbr_dir) {
            ...
            for (i = 0; i < curr_dir->size; i++) {
                ...
                if (i == curr_dir->size) {
                    printf("from find file or folder : we do not find the next step of "
                           "the path\n");
                    return -1;
                }
            }
...
int open_rec_dir(myDIR *curr_dir) {
    if (curr_dir->size == 0) {
        printf("from open_rec : the folder is empty there is no need to open "
               "something\n");
        return -1;
    }
    for (int i = 0; i < curr_dir->size; i++) {
